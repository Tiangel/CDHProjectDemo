...
private[spark] class DirectKafkaInputDStream[K, V](
  ...
  val repartitionStep = _ssc.conf.getInt("per.partition.offsetrange.step", 1000)
  val repartitionThreshold = _ssc.conf.getInt("per.partition.offsetrange.threshold", 1000)
  val enableRrepartition = _ssc.conf.getBoolean("enable.auto.repartition", false)
  ...
  override def compute(validTime: Time): Option[KafkaRDD[K, V]] = {
    ...
    val offsetRanges = untilOffsets.flatMap { case (tp, uo) =>
      ...
      val delta = uo - fo
      if (enableRrepartition && (repartitionThreshold < delta)) {
