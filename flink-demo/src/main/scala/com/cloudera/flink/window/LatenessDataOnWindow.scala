...
import com.cloudera.flink.function.TestAggregatFunctionByWindow.streamEnv
import com.cloudera.flink.source.StationLog
...
class LatenessDataOnWindow extends App {
  ...
  val stream: DataStream[StationLog] = streamEnv.socketTextStream(hostname, port)
    .map(line => {
      var arr = line.split(",")
  ...
  streamEnv.execute("Lateness Data On Window Job")
}
