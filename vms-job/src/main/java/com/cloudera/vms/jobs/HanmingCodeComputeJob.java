...
public  class HanmingCodeComputeJob {
	
  private static final Pattern SPACE = Pattern.compile(" ");
  private static final Logger logger =  LogManager.getLogger(HanmingCodeComputeJob.class);
  
  private HanmingCodeComputeJob() {
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    SparkConf sparkConf = new SparkConf().setAppName("HanmingCodeComputeJob");
...
//                	}else{
                	...
                	//}
                		content=	content.replaceAll("(//@[\\S]+:)|(@[\\S]+\\s)|(\\[花心\\])", "");
                	 if(content.length()>140){
                		 content = content.substring(0,140);
                	 }
                     return RowFactory.create(mid, WordSegmentationUtils.getTerms(content),null,content);  
                 }  
             });
...
//				  List<Row> rows = jrdd.collect();
//				  Set<String> ids = new HashSet<String>();
//				  for(Row r:rows){
//					  if(ids.contains(r.getString(0))){
//						  System.out.println("@@@@@@@@@@@@@@@@@@@@@@@duplicate id:"+r.getString(0));
//					  }
...
//				  }
			...
			JavaRDD<EsWeibo> rddHanmings = rddRows.map(new Function<Row, EsWeibo>() {
             public EsWeibo call(Row r) { 
          		  ...
          		  try{
       			  ...
       			  //System.out.println("id:"+id+",hanmingCode:"+Long.toUnsignedString(hanmin, 16));
			...
			JavaPairRDD<?,?>  esData = rddRows.mapToPair(new PairFunction<Row,Object,Object>(){
				...
				public Tuple2<Object, Object> call(Row t) {
					SparseVector feature = (SparseVector)t.getAs(0);
					try{
						 Field field=SparseVector.class.getDeclaredField("indices"); 
	            		  field.setAccessible(true);
	            		int[] indices = (int[]) field.get(feature);
						...
						Map<String, ?> data = ImmutableMap.of("hanmingCode",hanming);
			...
			rddHanmings.foreachPartition(new VoidFunction<Iterator<EsWeibo>>(){
				...
				public void call(Iterator<EsWeibo> hanmingIterator)
					...
					HBaseJavaAPI.addRows(Config.get(Config.KEY_HBASE_WEIBO_NAME), rowBeans);
			...
			JavaEsSpark.saveToEsWithMeta(esData, Config.get(Config.KEY_ES_WEIBO_URL));
