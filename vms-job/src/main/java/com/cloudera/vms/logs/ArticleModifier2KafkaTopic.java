...
public class ArticleModifier2KafkaTopic {

	private static final Logger logger = LogManager.getLogger(ArticleModifier2KafkaTopic.class);
			...
			public void run() {
				...
				while (true) {
					try {
							...
							if (sends.size() > 400 || now - lastCommitTime > 5000) {
								for (Future<RecordMetadata> s : sends) {
									try {
										...
										logger.info("send msg to update_data failed");
										e.printStackTrace();
									}
						...
						logger.info("#########commit to kafka update_data error");
						e.printStackTrace();
					}
				}
			}
	...
	public static void sendNews(String news) {
		...
		ProducerRecord<String, String> data = new ProducerRecord<String, String>(
				Config.get(Config.KEY_KAFKA_TOPIC_UPDATE), news);

		Future<RecordMetadata> send = producer.send(data);
		try {
			queue.put(send);
		} catch (Exception e) {
			logger.info("send messages to kafka topic update_data failed");
			e.printStackTrace();
